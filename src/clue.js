...
const suspectsArray = [mrMustard, mrsPeacock, missScarlet, profPlum, drOrchidconst, mrGreen];
...
const knife = { name: 'knife', weight: 10 };
...
// ITERATION 2
const randomStack = suspectsArray.concat(weaponsArray, roomsArray);

function selectRandom(someArray) {
    return (someArray[Math.floor(Math.random() * someArray.length)]);
};
selectRandom(randomStack);

function pickMystery() {
    const randomSuspect = Math.floor(Math.random() * suspectsArray.length);
    const randomWeapon = Math.floor(Math.random() * weaponsArray.length);
    const randomRoom = Math.floor(Math.random() * roomsArray.length);
    const mystery = { suspect: suspectsArray[randomSuspect], weapon: weaponsArray[randomWeapon], room: roomsArray[randomRoom] };
    return mystery;
}
